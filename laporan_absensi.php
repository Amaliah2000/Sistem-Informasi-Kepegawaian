...
$tampil=mysqli_query($conn, "select * from pegawai,jabatan,bagian where pegawai.id_jab=jabatan.id_jab and
pegawai.id_bag=bagian.id_bag");
$cekabsen=mysqli_query($conn, "select * from absensi where Month(tanggal_absen)='$_POST[bulan]'
		and Year(tanggal_absen)='$_POST[tahun]'");
$cek=mysqli_num_rows($cekabsen);
if($cek>0){
  ...
  while($dt=mysqli_fetch_array($tampil)){
	$absen=mysqli_query($conn,"select * from absensi where Month(tanggal_absen)='$_POST[bulan]'
		and Year(tanggal_absen)='$_POST[tahun]' and status_masuk='Y' and status_keluar='Y' and nip='$dt[nip]'");
	$jml=mysqli_num_rows($absen);
	$telat=mysqli_query($conn,"select * from absensi where Month(tanggal_absen)='$_POST[bulan]'
		and Year(tanggal_absen)='$_POST[tahun]' and terlambat='Y' and nip='$dt[nip]'");
	$izin=mysqli_query($conn,"select * from absensi where Month(tanggal_absen)='$_POST[bulan]'
		and Year(tanggal_absen)='$_POST[tahun]' and ket='I' and nip='$dt[nip]'");
	
	$sakit=mysqli_query($conn,"select * from absensi where Month(tanggal_absen)='$_POST[bulan]'
		and Year(tanggal_absen)='$_POST[tahun]' and ket='S' and nip='$dt[nip]'");
	
	$tot_telat=mysqli_num_rows($telat);
	$tot_izin=mysqli_num_rows($izin);
	$tot_sakit=mysqli_num_rows($sakit);
