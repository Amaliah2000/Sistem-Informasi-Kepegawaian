...
$ambil=mysqli_query($conn, "select * from pegawai where nip='$_GET[id]'");
	...
	$bag=mysqli_query($conn,"select * from bagian where id_bag='$t[id_bag]'");
	...
	$ri=mysqli_query($conn, "select * from pendidikan where nip='$_GET[id]' order by idp ASC");
	...
	$nip=$_SESSION['namauser'];
	$ri=mysqli_query($conn,"select * from pengalaman_kerja where nip='$_GET[id]' order by id_peker ASC");
	if(mysqli_num_rows($ri)==0){
	echo "<tr>
	<td colspan='2'>*Tidak Ada Data*</td>
	</tr>";
